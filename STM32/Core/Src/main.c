...
int main(void)
{
  ...
  joint_t shoulder = {.htimer = &htim5, .CCRReg = &(TIM5->CCR1), .channel = TIM_CHANNEL_1, .min_angle = 70, .max_angle = 150};
  ...
  while (1)
  {
    //joint_set_angle(arm.hand, angle);
    //if(up){
    //  angle++;
    //  if(angle >= 180){
    //    up = false;
    ...
    //  }
    //}
    arm_set_angles(&arm, USB_RX_Buf, 20);
    HAL_Delay(30);
